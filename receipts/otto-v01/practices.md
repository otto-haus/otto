...
- **What changed:** `@otto-do/practices`; bin `vinny-practices` → `otto-practices`;
  `cli.ts` now `OTTO_DO_ROOT ?? VINNY_OS_ROOT`; `validate.test.ts` made portable (resolves repo root, no hardcoded path).
