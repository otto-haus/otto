 ...
 *
 * Embedded ("This Mac") is otto-owned: there is no separate Letta.app to launch, so we reveal
 * the embedded runtime home instead of blindly opening `/Applications/Letta.app`. The
 * existing/cloud (advanced) path prefers an installed Letta Desktop and falls back to docs.
 */
export function planOpenLettaTarget(opts: PlanOpenLettaOptions): OpenLettaTarget {
  if (opts.connectionMode === 'embedded') {
    return {
      ...
      reason:
        'Embedded (This Mac) runtime is supervised by otto — opening its local runtime folder, not Letta.app.',
    };
  }
